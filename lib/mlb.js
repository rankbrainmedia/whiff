...
export async function fetchTeamHittingStats(season) {
  ...
  return sorted.map((s, i) => ({
    ...
    obp: s.stat?.obp ?? null,
    teamKPct: s.stat?.plateAppearances
      ? (s.stat?.strikeOuts ?? 0) / s.stat.plateAppearances
      : null,
  }));
}
...
// Fetch pitcher K% by batter handedness (vs LHB and vs RHB)
export async function fetchPitcherHandednessSplits(pitcherId, season) {
  const yr = season || new Date().getFullYear();
  try {
    const url = `${BASE}/people/${pitcherId}/stats?stats=statSplits&group=pitching&season=${yr}&sitCodes=vl,vr`;
    const res = await fetch(url);
    if (!res.ok) return { vsL: null, vsR: null };
    const data = await res.json();
    const splits = data.stats?.[0]?.splits ?? [];
    ...
    const kPctFromSplit = (split) => {
      if (!split) return null;
      const bf = split.stat?.battersFaced ?? 0;
      const k  = split.stat?.strikeOuts ?? 0;
      return bf >= 10 ? k / bf : null;
    };

    return {
      vsL: kPctFromSplit(vsL),
      vsR: kPctFromSplit(vsR),
      vsLBF: vsL?.stat?.battersFaced ?? 0,
      vsRBF: vsR?.stat?.battersFaced ?? 0,
    };
  } catch {
    return { vsL: null, vsR: null, vsLBF: 0, vsRBF: 0 };
  }
}