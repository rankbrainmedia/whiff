...
export function computeParkAdj(kHatRaw, venueName) {
  // Fuzzy match — MLB API returns full names like "Oriole Park at Camden Yards"
  let factor = 1.0;
  if (venueName) {
    const vLower = venueName.toLowerCase();
    for (const [park, f] of Object.entries(MODEL_CONFIG.park_k_factors)) {
      if (vLower.includes(park.toLowerCase()) || park.toLowerCase().includes(vLower)) {
        factor = f;
        break;
      }
    }
...
export function computeProjectionV2({
  ...
  // v3: new signals
  sameOpponentAnchor, // { anchor, weight, games } — applied last as reality anchor
  teamKHotCold,       // { modifier, gamesUsed, ratio } — additive K adjustment
  scoutBFModifier,    // int [-5..+5] from LLM scout
  scoutKPctModifier,  // float [-0.05..+0.05] from LLM scout
  ...
  // Scout BF modifier — applied after all other BF adjustments
  if (scoutBFModifier != null && scoutBFModifier !== 0) {
    bfHat = Math.max(9, bfHat + scoutBFModifier);
  }

  // Scout K% modifier — clamp to safe range, apply to all K% inputs before calculations
  const kPctDelta = (scoutKPctModifier != null)
    ? Math.max(-0.05, Math.min(0.05, scoutKPctModifier))
    : 0;
  const effectiveKPct   = pitcherKPct   != null ? Math.max(0, Math.min(1, pitcherKPct   + kPctDelta)) : pitcherKPct;
  const effectiveKPctVsL = pitcherKPctVsL != null ? Math.max(0, Math.min(1, pitcherKPctVsL + kPctDelta)) : pitcherKPctVsL;
  const effectiveKPctVsR = pitcherKPctVsR != null ? Math.max(0, Math.min(1, pitcherKPctVsR + kPctDelta)) : pitcherKPctVsR;
  ...
  if (lineup?.length && kRateMap && Object.keys(kRateMap).length > 0) {
    ...
    const lineupK = computeLineupExpectedK({
      lineup, kRateMap,
      pitcherKPct: effectiveKPct,
      pitcherKPctVsL: effectiveKPctVsL,
      pitcherKPctVsR: effectiveKPctVsR,
      bfHat,
    });
    kHatRaw = lineupK ?? computeTeamLevelK(effectiveKPct, teamKFactor, bfHat);
  } else {
    mode = 'B';
    kHatRaw = computeTeamLevelK(effectiveKPct, teamKFactor, bfHat);
  }
  ...
  const dHotCold = teamKHotCold?.modifier ?? 0; // team hot/cold signal

  let kHat = kHatRaw + dStuff + dUmp + dPark + dHotCold;

  // Same-opponent anchor — applied LAST as a reality anchor on observed results
  if (sameOpponentAnchor?.weight && sameOpponentAnchor.anchor != null) {
    kHat = (1 - sameOpponentAnchor.weight) * kHat + sameOpponentAnchor.weight * sameOpponentAnchor.anchor;
  }
  ...
  return {
    ...
    dHotCold: Math.round(dHotCold * 100) / 100,
    signal,
